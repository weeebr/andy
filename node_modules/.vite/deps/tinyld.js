...
// node_modules/.pnpm/tinyld@1.3.4/node_modules/tinyld/dist/tinyld.normal.node.mjs
...
function E(a) {
  return a in u ? u[a].region : "";
}
function G(a) {
  return a in u ? u[a].name : "";
}
function I(a) {
  let i = Object.entries(u).find((o) => o[1].code === a);
  if (i) return i[1].code;
  let n = Object.entries(u).find((o) => o[1].alias && o[1].alias.includes(a));
  return n ? n[1].code : "";
}
function m(a) {
...
function Z(a, i, n = true) {
  ...
  for (let B = 0; B < t.length - (i - 1); B++) {
    let r = [], s = 0;
    for (let e = 0; e < i; e++) t[B + e] === " " ? s += 1 : s = 0, !(s > 1) && r.push(t[B + e]);
    let O = r.join("");
...
function w(a, i, n, o) {
  for (let t of F) {
    ...
    for (let r of B) {
      if (!n.has(r)) continue;
      let s = m(i.uniques[r]);
      if (!(o.only.length > 0 && !o.only.includes(s))) return o.verbose && console.log(`[Pass 1] detectUniqueGrams ${t}-grams - match '${r}' to ${s}`), s;
    }
  }
  return "";
}
function x(a, i, n) {
  ...
  for (let e of t) {
    ...
    for (let L of B) d.has(L) && (o.set(L, (o.get(L) || 0) + D[L] * l.length / 4), A.push(`${L} = ${D[L] / 1024 * 100}%`));
...
function Y(a, i, n, o) {
  ...
  for (let e of t) {
    ...
    let D = W(e);
    for (let d of D) x(d, n, i).forEach((L) => {
...
//# sourceMappingURL=tinyld.js.map
